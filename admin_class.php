...
Class Action {
	...
	function save_staff(){
		...
		$check = $this->db->query("SELECT * FROM report WHERE report_title ='$report_title' " . (!empty($id) ? " AND id != {$id} " : ''))->num_rows;
		if($check > 0){
			return 2;
			exit;
		}
		if(empty($id)){
			$save = $this->db->query("INSERT INTO report set $data");
		}else{
			$save = $this->db->query("UPDATE report set $data where id = $id");
		}
	...
	function delete_staff(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM report where id = ".$id);
		if($delete){
			return 1;
		}
	}
	...
	function save_report() {
		...
		// تحقق من وجود تقرير بنفس العنوان (مثال للتحقق من التكرار)
		$check = $this->db->query("SELECT * FROM report WHERE report_title ='$report_title' " . (!empty($id) ? " AND id != {$id} " : ''))->num_rows;
		if ($check > 0) {
			return 2;
			exit;
